 ...
 *
 * built ins are split into forkable and unforkable, main use is to not run any unforkable commands in a pipe.
 */
static int push_built_ins(t_shell* shell){
    ...
    if (insert_builtin(&(shell->builtins), "unalias", unalias_builtin, 0) == NULL) {
    ...
    if (insert_builtin(&(shell->builtins), "history", history_builtin, 1) == NULL) {
 ...
 * @param shell pointer to shell struct
 *
 * Initializes to either malloc of initial lengths, defined within shell_init.h, or NULL / -1 values.
 *
 * @note mallocd env freed in shell_cleanup.h, called atexit()
*/
int init_shell_state(t_shell* shell){
    ...
    if(shell->is_interactive && shell->job_control_flag && shell->tty_fd != -1 && shell->tty_fd != STDIN_FILENO){
