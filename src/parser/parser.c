...
static t_ast_n* parse_pipeline(t_ast* ast, t_token_stream* ts, int start, int end); ///< Forward declaration of parse pipeline
static t_ast_n* parse_conditionals(t_ast* ast, t_token_stream* ts, int start, int end); ///< Forward declaration of parse conditionals
static t_ast_n* parse_subshells(t_ast* ast, t_token_stream* ts, int start, int end); ///< Forward declaration
...
static int handle_realloc_io_redir(t_ast_n* node, size_t* capacity){
    ...
    t_io_redir** new_redir = realloc(node->io_redir, sizeof(t_io_redir*) * new_cap);
    if(!new_redir){
 ...
 *
 * Finds all redirection tokens within command argv from start to end parsing them into a node.
 * start and end defined by function parse_pipeline, which calls recursively.
 */
static int scan_redirections(t_ast_n* node, t_token_stream* token_stream, int start, int end){

    node->io_redir = (t_io_redir**)malloc(sizeof(t_io_redir*) * INITIAL_REDIR_LEN);
    if(!node->io_redir){
 ...
 * then parses the remaining command struct argv (start to end) into node argv, returning node.
 */
static t_ast_n* parse_command(t_ast* ast, t_token_stream* ts, int start, int end){
...
static int next_terminator_index(t_token_stream* ts, int start, int end){
    ...
    for(int i = start; i <= end; i++){
        ...
        if(ts->tokens[i].type == TOKEN_OPEN_PAR)
            par_depth++;
        if(ts->tokens[i].type == TOKEN_CLOSE_PAR)
            par_depth--;

        if(par_depth == 0 && (ts->tokens[i].type == TOKEN_SEQ || ts->tokens[i].type == TOKEN_BG))
            return i;
    }
    return -1;
}
 ...
 */
static t_ast_n* parse_terminators(t_ast* ast, t_token_stream* ts, int start, int end){
 ...
 *
 * Searches command argv range start to end accordingly, looking and parsing conditional operators and breaking them apart
 * into subtrees returned to parse_terminators to attach to AST.
 */
static t_ast_n* parse_conditionals(t_ast* ast, t_token_stream* ts, int start, int end){
    ...
    if(last_conditional_index == -1){
        ...
        if(!node->left || !node->right){
            ...
            if(node->left) cleanup_ast(node->left);
            if(node->right) cleanup_ast(node->right);
...
/**
 * @brief recursively parses pipeline tokens and their respective left and right children
 * @param ast pointer to ast to parse into
 * @param command pointer to command struct
 * @param start start index of command argv
 * @param end end index of command argv
 ...
 *  - Parse left and right subtrees recursively via parse_command (which calls scan_redirections)
 * This creates a left-associative AST to be given to the executor.
 *
 */
static t_ast_n* parse_pipeline(t_ast* ast, t_token_stream* ts, int start, int end){
...
static t_ast_n* parse_subshells(t_ast* ast, t_token_stream* ts, int start, int end){
    ...
    if(first_open_par_idx == -1 || last_close_par_idx == -1){
        ...
        node->sub_ast_root = parse_terminators(ast, ts, first_open_par_idx + 1, last_close_par_idx - 1);
...
/**
 * @brief function to be called by the parse_and_execute function defined in executor.h
 * @param ast pointer to ast to parse into
 * @param command pointer to command struct
 *
 * @return ast root node on success, NULL on fail.
...
t_ast_n* build_ast(t_ast* ast, t_token_stream* token_stream){
    ...
    init_ast(ast);
    if((ast->root = parse_terminators(ast, token_stream, 0, token_stream->tokens_arr_len - 1)) == NULL){
        return NULL;
    }
