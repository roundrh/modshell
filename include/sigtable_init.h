 ...
 *
 * sigtable's sig initialized with zero vals via memset then sigemptyset to safely init an empty set for sa_mask
 * sigaction to set sig handler for newact in child struct (sigstruct.h for more info)
 * saves default into child struct oldact
 * 
 */
...
do{                                                                              \
    ...
    sigtable->sig.newact.sa_handler = handler;                                   \
                                                                                 \
    if(sigaction(sig_c, &(sigtable->sig.newact), &(sigtable->sig.oldact)) == -1){\
        perror("sigaction");                                                     \
        return -1;                                                               \
    }                                                                            \
}while(0)                                                                        
...
/**
 * @brief initializes sigtable parent struct with values matching specific signals.
 * @param sigtable pointer to shell sigtable parent struct
 *
 * @return 0 on success, -1 on err. 
 *
 * Function calls above macro (INIT_SIG) on all signals whose handlers are set to non-default handlers.
 */
