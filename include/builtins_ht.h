...
typedef int (*t_builtin_func)(t_ast_n* node, t_shell* shell, char** argv); ///< function pointer to builtin function

/**
 * @typedef s_ht_node t_ht_node
 * @brief encapsulated data for a node in the chains within hashtable buckts
 *
 * Struct contains data about the ht's node such as the key (command alias), function pointer to built in
 * forkability (boolean value showing if the builtin is forkable or must run in parent), and next pointer for list.
 */
